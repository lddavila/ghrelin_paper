function [] = create_migration_plots(all_dirs,feature,directory_where_overlay_plots_should_be_saved_relative,pdfs_dir)
    function [current_axis_object] = plot_data_overlap(all_sigmoid_info1,all_sigmoid_info2,feature,experiment1,experiment2,directory_where_plots_should_be_saved_absolute,pdfs_dir)
        ...
        for j=1: length(all_experiment_1_data_together)
            f = figure;
            [best_mpc1,all_scatter_objects,all_marker_objects] = create_a_plot_updated_2(f,all_experiment_1_data_together{j},labels1,all_x_labels(j),all_y_labels(j),feature,experiment1,"","create_overlay_plots.m",directory_where_plots_should_be_saved_absolute);
...
directory_where_overlay_plots_should_be_saved_absolute = cd(homeDir);
...
for i=1:height(all_dirs)
    ...
    experiment1 = split(experiment1," ");
    feature_1 = experiment1(2);
    ...
    %% create a subtable from all_dirs with only feature 1 and experiments which are not experiment 1
    only_feature_1_and_not_experiment_1 = all_dirs(~contains(all_dirs.Experiment,experiment1) & contains(all_dirs.Experiment,feature_1),:);

    for second=1:height(only_feature_1_and_not_experiment_1)
        experiment2 = only_feature_1_and_not_experiment_1{second,1};
        experiment2 = split(experiment2," ");
        feature_2 = experiment2(2);
        ...
        current_axis_object= plot_data_overlap(all_sigmoid_info1,all_sigmoid_info2,feature,experiment1,experiment2,directory_where_overlay_plots_should_be_saved_absolute,pdfs_dir_absolute);
